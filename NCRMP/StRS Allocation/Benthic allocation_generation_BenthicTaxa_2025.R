...
# Load libraries
library(tidyverse) # for summarizing/organizing data
library(dplyr)     # for summarizing/organizing data- LOAD LAST TO PREVENT MASKING
library(ggrepel)
...
STR_ALLOC_ALLSC=NULL
for(SCENARIO in c("UNPOOLED_SEC","Pooled_MPA_SGA")){
  if(SCENARIO=="Pooled_MPA_SGA"){
    ...
    strataarea=strataarea_r %>%
      ...
      summarise(AREA_HA_2023=sum(AREA_HA_2023)) %>% 
      na.omit() 
  }else{
    REG=rbind(REG1,REG2) %>%
      mutate(ANALYSIS_YR_NUM=as.numeric(AYNlu[ANALYSIS_YEAR])) %>% 
  ...
  # join totals back to original data frame to calculate proportion
  STR.GEN_pSD<-REG %>% 
  ...
  # get area values for each strata from the 'strataarea' dataframe
  STR_pSD_A<- strataarea %>% dplyr::select(REGION,ISLAND, SEC_NAME, REEF_ZONE, DEPTH_BIN, AREA_HA_2023)%>% 
  ...
  ISL_AREA<-strataarea %>% group_by(REGION,ISLAND) %>%
    dplyr::summarize(ISLAND_AREA=sum(AREA_HA_2023,na.rm=T)) # summarize area by island
  REG_AREA<-strataarea %>% group_by(REGION) %>%
    dplyr::summarize(REGION_AREA=sum(AREA_HA_2023,na.rm=T)) # summarize area by island
  
  STR_pSD_pA_prodSDA<-STR_pSD_A %>% 
    left_join(ISL_AREA,by=c("REGION","ISLAND")) %>% 
    left_join(REG_AREA,by=c("REGION")) %>% 
    mutate(pAREA_ISL=AREA_HA_2023/ISLAND_AREA,
           pAREA_REG=AREA_HA_2023/REGION_AREA) %>% 
  ...
  STR_pSDA<-STR_pSD_pA_prodSDA %>% 
    ...
    dplyr::select(REGION,ISLAND, SEC_NAME, REEF_ZONE, DEPTH_BIN,
                  pSD_ISL,pSD_REG,pAREA_ISL,pAREA_REG,pSDA_ISL,pSDA_REG) # clean up dataframe
  ...
  SpI=read.csv("T:/Benthic/Data/StRS Allocation/Draft Itinerary NCRMP-MA FY25 - SiteAllocationPivotTable_20250326.csv") %>% 
  ...
  SpI$REGION[SpI$ISLAND=="Anatahan"]="NMARIAN"
  
  #If in pooling mode, pool SGA
  if (SCENARIO=="Pooled_MPA_SGA"){
    SpI$ISLAND[SpI$ISLAND%in%SGA2POOL]="Sarigan_Guguan_Alamagan"
    SpI=SpI %>% group_by(REGION,ISLAND) %>% summarize(TOTAL_SITES=sum(TOTAL_SITES))
  }
  
  RandomSiteReductionFactor=.58
  SpI$SfM_SITES=round(SpI$TOTAL_SITES*RandomSiteReductionFactor)
  SpI_REGION=SpI %>% group_by(REGION) %>% summarize(SfM_SITES=sum(SfM_SITES))
  
  #All Sectors (including Maui Extras)
  STR_ALLOC=STR_pSDA %>% 
    left_join(SpI,by=c("REGION","ISLAND")) %>% 
    mutate(SDxA_ALLOC_ISL=round(SfM_SITES*pSDA_ISL),
           AREA_ALLOC_ISL=round(SfM_SITES*pAREA_ISL))  
  ...
  REG_History=REG_History %>% ungroup() %>% 
    select(ISLAND,SEC_NAME,REEF_ZONE,DEPTH_BIN,N_2014,N_2017,N_2022) %>% 
    arrange(ISLAND,SEC_NAME,REEF_ZONE,DEPTH_BIN)
  
  
  #Add History
  STR_ALLOC=STR_ALLOC %>% left_join(REG_History,by=join_by(ISLAND, SEC_NAME, REEF_ZONE, DEPTH_BIN))
  ...
  # save file
  print(paste0("Total Survey Sites = ",sum(STR_ALLOC$SDxA_ALLOC_ISL),"; With Random Site Reduction Factor = ",RandomSiteReductionFactor))
  print(paste0("Historical Totals = ",paste0(apply(X = STR_ALLOC[,c("N_2014","N_2017","N_2022")],2,sum),collapse = ", ")))
  write.csv(STR_ALLOC,file=paste0("./NCRMP25_SITES_Allocation_",Sys.Date(),"_",SCENARIO,".csv"),row.names = FALSE)
  
  #All Scenarios
  STR_ALLOC_ALLSC=rbind(STR_ALLOC_ALLSC,STR_ALLOC) %>% distinct()
}
STR_ALLOC_ALLSC.=STR_ALLOC_ALLSC %>% ungroup() %>% 
  mutate(RZ_DB=paste0(substr(REEF_ZONE,1,1),substr(DEPTH_BIN,1,1)),
         COMBO=toupper(str_trim(paste0(ISLAND,"_",SEC_NAME,"_",RZ_DB)))) %>% 
  select(REGION,ISLAND,SEC_NAME,RZ_DB,COMBO,TOTAL_SITES,SfM_SITES,SDxA_ALLOC_ISL,N_2014,N_2017,N_2022,AREA_ALLOC_ISL,pSD_ISL,pAREA_ISL,pSDA_ISL) %>% 
  distinct()
write.csv(STR_ALLOC_ALLSC.,file=paste0("./NCRMP25_SITES_Allocation_",Sys.Date(),"_ALL_SCENARIOS.csv"),row.names = FALSE)
# 
# 
# # #Select Target Taxa: ----------------------------------------------------
