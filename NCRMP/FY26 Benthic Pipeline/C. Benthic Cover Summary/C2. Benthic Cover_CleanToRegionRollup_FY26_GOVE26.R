...
#Drop sites not in survey master before join
SVIDdrop=as.numeric(na.omit(setdiff(wsd$SITEVISITID,sm$SITEVISITID)))
wsd=wsd %>% filter(!SITEVISITID%in%SVIDdrop,!is.na(SITEVISITID))

#Join strategic cols in sm
smcol=c("SITEVISITID","MISSIONID","OBS_YEAR","DATE_","SEC_NAME","new_MIN_DEPTH_M","new_MAX_DEPTH_M",
        "LATITUDE_LOV","LONGITUDE_LOV","ANALYSIS_YEAR","ANALYSIS_SCHEME",
        "TRANSECT_PHOTOS", "EXCLUDE_FLAG" , "Oceanography","OCC_SITEID")
wsd_t1<-left_join(wsd,sm[,smcol],by=c("SITEVISITID","OBS_YEAR"))#intersect(names(wsd),names(sm))#
grep(pattern = ".x",names(wsd_t1),value = T)
which(wsd_t1$PERM_SITE.x!=wsd_t1$PERM_SITE.y)
...
#dim(wsd);dim(wsd.)
#Make sure that you have the correct # of sites/region and year
test1<-ddply(wsd,.(REGION,OBS_YEAR),summarize,nSite_wsd=length(unique(SITE))) %>% arrange(OBS_YEAR,REGION)
...
smcol=c("SITEVISITID","MISSIONID","OBS_YEAR","DATE_","SEC_NAME","new_MIN_DEPTH_M","new_MAX_DEPTH_M",
        "LATITUDE_LOV","LONGITUDE_LOV","ANALYSIS_YEAR","ANALYSIS_SCHEME",
        "TRANSECT_PHOTOS", "EXCLUDE_FLAG" , "Oceanography","OCC_SITEID")
wsd_t2<-left_join(wsd,sm[,smcol],by=c("SITEVISITID","OBS_YEAR"))
...
wsd_t2 %>% filter(is.na(ANALYSIS_YEAR))
head(wsd_t2)
...
smcol=c("SITEVISITID","MISSIONID","OBS_YEAR","DATE_","SEC_NAME","new_MIN_DEPTH_M","new_MAX_DEPTH_M",
        "LATITUDE_LOV","LONGITUDE_LOV","ANALYSIS_YEAR","ANALYSIS_SCHEME",
        "TRANSECT_PHOTOS", "EXCLUDE_FLAG" , "Oceanography","OCC_SITEID")
wsd_t3<-left_join(wsd,sm[,smcol],by=c("SITEVISITID","OBS_YEAR"))
...
for (TIER in 1:3){#TIER=1#2#2#
  ...
  if(WRITE){
    ...
    write.csv(wsd,paste0(VizToolOutputDir,"2024ViztoolSites_Cover.csv"))
    }
  ...
  if(TIER==1){
    ...
    if(WRITE){write.csv(dpstC, file=paste0(CoreOutputDir,"Stratum/BenthicCover_2010-2024_Tier2b_STRATA_Complete.csv"),row.names = F)}
    if(WRITE){write.csv(dpstT, file=paste0(CoreOutputDir,"Stratum/BenthicCover_2010-2024_Tier2b_STRATA_Trends.csv"),row.names = F)}
    if(WRITE){write.csv(dpstC, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier2b_STRATA_Complete_Viztool.csv"),row.names = F)}
    if(WRITE){write.csv(dpstT, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier2b_STRATA_Trends_Viztool.csv"),row.names = F)}
  }else{
    if(WRITE){write.csv(dpstC, file=paste0(CoreOutputDir,"Stratum/BenthicCover_2010-2024_Tier3_STRATA_Complete.csv"),row.names = F)}
    if(WRITE){write.csv(dpstT, file=paste0(CoreOutputDir,"Stratum/BenthicCover_2010-2024_Tier3_STRATA_Trends.csv"),row.names = F)}
    if(WRITE){write.csv(dpstC, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier3_STRATA_Complete_Viztool.csv"),row.names = F)}
    if(WRITE){write.csv(dpstT, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier3_STRATA_Trends_Viztool.csv"),row.names = F)}
  }
...
#  dpsecT$Mean==dpsecT_2025$Mean
  ...
  if(TIER==1){
    ...
    if(WRITE){write.csv(dpsecC, file=paste0(CoreOutputDir,"Sector/BenthicCover_2010-2024_Tier2b_SECTOR_Complete.csv"),row.names = F)}
    if(WRITE){write.csv(dpsecT, file=paste0(CoreOutputDir,"Sector/BenthicCover_2010-2024_Tier2b_SECTOR_Trends.csv"),row.names = F)}
    if(WRITE){write.csv(dpsecC, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier2b_SECTOR_Complete_Viztool.csv"),row.names = F)}
    if(WRITE){write.csv(dpsecT, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier2b_SECTOR_Trends_Viztool.csv"),row.names = F)}
  }else{
    if(WRITE){write.csv(dpsecC, file=paste0(CoreOutputDir,"Sector/BenthicCover_2010-2024_Tier3_SECTOR_Complete.csv"),row.names = F)}
    if(WRITE){write.csv(dpsecT, file=paste0(CoreOutputDir,"Sector/BenthicCover_2010-2024_Tier3_SECTOR_Trends.csv"),row.names = F)}
    if(WRITE){write.csv(dpsecC, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier3_SECTOR_Complete_Viztool.csv"),row.names = F)}
    if(WRITE){write.csv(dpsecT, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier3_SECTOR_Trends_Viztool.csv"),row.names = F)}
  }
  ...
  if(TIER==1){
    ...
    if(WRITE){write.csv(dpisC, file=paste0(CoreOutputDir,"Island/BenthicCover_2010-2024_Tier2b_ISLAND_Complete.csv"),row.names = F)}
    if(WRITE){write.csv(dpisT, file=paste0(CoreOutputDir,"Island/BenthicCover_2010-2024_Tier2b_ISLAND_Trends.csv"),row.names = F)}
    if(WRITE){write.csv(dpisC, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier2b_ISLAND_Complete_Viztool.csv"),row.names = F)}
    if(WRITE){write.csv(dpisT, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier2b_ISLAND_Trends_Viztool.csv"),row.names = F)}
  }else{
    if(WRITE){write.csv(dpisC, file=paste0(CoreOutputDir,"Island/BenthicCover_2010-2024_Tier3_ISLAND_Complete.csv"),row.names = F)}
    if(WRITE){write.csv(dpisT, file=paste0(CoreOutputDir,"Island/BenthicCover_2010-2024_Tier3_ISLAND_Trends.csv"),row.names = F)}
    if(WRITE){write.csv(dpisC, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier3_ISLAND_Complete_Viztool.csv"),row.names = F)}
    if(WRITE){write.csv(dpisT, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier3_ISLAND_Trends_Viztool.csv"),row.names = F)}
  }
  ...
  if(TIER==1){
    ...
    if(WRITE){write.csv(dprC, file=paste0(CoreOutputDir,"Region/BenthicCover_2010-2024_Tier2b_REGION_Complete.csv"),row.names = F)}
    if(WRITE){write.csv(dprT, file=paste0(CoreOutputDir,"Region/BenthicCover_2010-2024_Tier2b_REGION_Trends.csv"),row.names = F)}
    if(WRITE){write.csv(dprC, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier2b_REGION_Complete_Viztool.csv"),row.names = F)}
    if(WRITE){write.csv(dprT, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier2b_REGION_Trends_Viztool.csv"),row.names = F)}
  }else{
    if(WRITE){write.csv(dprC, file=paste0(CoreOutputDir,"Region/BenthicCover_2010-2024_Tier3_REGION_Complete.csv"),row.names = F)}
    if(WRITE){write.csv(dprT, file=paste0(CoreOutputDir,"Region/BenthicCover_2010-2024_Tier3_REGION_Trends.csv"),row.names = F)}
    if(WRITE){write.csv(dprC, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier3_REGION_Complete_Viztool.csv"),row.names = F)}
    if(WRITE){write.csv(dprT, file=paste0(VizToolOutputDir,"BenthicCover_2010-2024_Tier3_REGION_Trends_Viztool.csv"),row.names = F)}
  }
