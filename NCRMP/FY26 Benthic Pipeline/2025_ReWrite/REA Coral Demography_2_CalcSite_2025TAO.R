...
#These data only include surveys conducted between 2013-2023
rm(list=ls())
...
source("./NCRMP/2025_ReWrite/Benthic_Functions_newApp_v2025TAOfork.R")
source("./NCRMP/2025_ReWrite/core_functions_2025.R")
...
#awd=subset(awd,ISLAND=="Swains"& OBS_YEAR=="2023")
...
awd_pre %>% filter(!is.na(SEVERITY_1)) %>% View()
...
#Calc_ColDen_Transect
# tic()
# acd.tax<-Calc_ColDen_Transect(data = awd,grouping_field = "TAXONCODE");
# acd.tax=acd.tax %>% rename(AdColCount=ColCount,
#                            AdColDen=ColDen,
#                            TRANSECTAREA_ad=TRANSECTAREA)
# # colnames(acd.tax)[colnames(acd.tax)=="ColCount"]<-"AdColCount";colnames(acd.tax)[colnames(acd.tax)=="ColDen"]<-"AdColDen";colnames(acd.tax)[colnames(acd.tax)=="TRANSECTAREA"]<-"TRANSECTAREA_ad"# calculate density at genus level as well as total
# toc()
# tic()
acd.tax.<-Calc_ColDen_Transect_25(data = awd,grouping_field = "TAXONCODE");
acd.tax.=acd.tax. %>% rename(AdColCount=ColCount,
                   AdColDen=ColDen,
                   TRANSECTAREA_ad=TRANSECTAREA)
acd.tax=acd.tax.;rm(list="acd.tax.")
# toc()
# str(acd.tax)
# str(acd.tax.)
# identical(acd.tax,acd.tax.)
# all.equal(acd.tax,acd.tax.)

# tic()
# jcd.tax<-Calc_ColDen_Transect(data=jwd,grouping_field = "TAXONCODE");
# jcd.tax=jcd.tax %>% rename(JuvColCount=ColCount,
#                            JuvColDen=ColDen,
#                            TRANSECTAREA_j=TRANSECTAREA)
# toc()
# tic()
jcd.tax.<-Calc_ColDen_Transect_25(data = jwd,grouping_field = "TAXONCODE");
jcd.tax.=jcd.tax. %>% rename(JuvColCount=ColCount,
                             JuvColDen=ColDen,
                             TRANSECTAREA_j=TRANSECTAREA)
jcd.tax=jcd.tax.;rm(list="jcd.tax.")
# toc()
# identical(jcd.tax,jcd.tax.)
# all.equal(jcd.tax,jcd.tax.)
# str(jcd.tax)
# str(jcd.tax.)

#Calc_ColMetric_Transect
tic()
cl.tax<-Calc_ColMetric_Transect(data = awd,grouping_field = "TAXONCODE",pool_fields = "COLONYLENGTH");
colnames(cl.tax)[colnames(cl.tax)=="Ave.y"]<-"Ave.cl" #Average % old dead
cl.tax=cl.tax %>% arrange(METHOD,SITE,SITEVISITID,TRANSECT,TAXONCODE,Ave.cl)
toc()

tic()
cl.tax.<-Calc_ColMetric_Transect_25(data = awd,grouping_field = "TAXONCODE",pool_fields = "COLONYLENGTH") %>% 
  rename(Ave.cl=Ave.y) %>%
  arrange(METHOD,SITE,SITEVISITID,TRANSECT,TAXONCODE,Ave.cl)
toc()

identical(cl.tax,cl.tax.)
all.equal(cl.tax,cl.tax.)

str(cl.tax)
str(cl.tax.)

tic()
od.tax<-Calc_ColMetric_Transect(data = awd,grouping_field = "TAXONCODE",pool_fields = "OLDDEAD");
colnames(od.tax)[colnames(od.tax)=="Ave.y"]<-"Ave.od" #Average % old dead
od.tax=od.tax %>% arrange(METHOD,SITE,SITEVISITID,TRANSECT,TAXONCODE,Ave.od)
toc()
tic()
od.tax.<-Calc_ColMetric_Transect_25(data = awd,grouping_field = "TAXONCODE",pool_fields = "OLDDEAD") %>% 
  rename(Ave.od=Ave.y) %>%
  arrange(METHOD,SITE,SITEVISITID,TRANSECT,TAXONCODE,Ave.od)
toc()
identical(od.tax,od.tax.)

tic()
rd.tax<-Calc_ColMetric_Transect(data = awd,grouping_field = "TAXONCODE",pool_fields = c("RDEXTENT1", "RDEXTENT2","RDEXTENT3"));
colnames(rd.tax)[colnames(rd.tax)=="Ave.y"]<-"Ave.rd" #Average % recent dead
rd.tax=rd.tax %>% arrange(METHOD,SITE,SITEVISITID,TRANSECT,TAXONCODE,Ave.rd)
toc()
tic()
rd.tax.<-Calc_ColMetric_Transect_25(data = awd,grouping_field = "TAXONCODE",pool_fields = c("RDEXTENT1", "RDEXTENT2","RDEXTENT3")) %>% 
  rename(Ave.rd=Ave.y) %>%
  arrange(METHOD,SITE,SITEVISITID,TRANSECT,TAXONCODE,Ave.rd)
toc()
identical(rd.tax,rd.tax.)


#Calc_TotDZden_Transect
tic()
# Density of recent dead colonies by condition, you will need to subset which ever condition you want. The codes ending in "S" are the general categories
totdzden.tax<-Calc_TotDZden_Transect(data = awd,survey_colony_f = survey_colony,grouping_field = "TAXONCODE")
toc()
tic()
# Density of recent dead colonies by condition, you will need to subset which ever condition you want. The codes ending in "S" are the general categories
totdzden.tax.<-Calc_TotDZden_Transect_25(data = awd,survey_colony_f = survey_colony,grouping_field = "TAXONCODE") %>% 
  arrange(METHOD,SITE,SITEVISITID,TRANSECT,TAXONCODE)
toc()
identical(totdzden.tax,totdzden.tax.)
head(totdzden.tax)
head(totdzden.tax.)
...
#Calc_RDden_Transect
rdden.tax<-Calc_RDden_Transect(data = awd,survey_colony_f = survey_colony,grouping_field = "TAXONCODE") # Density of recent dead colonies by condition, you will need to subset which ever condition you want. The codes ending in "S" are the general categories
...
write.csv(site.data.gen2,file="T:/Benthic/Data/REA Coral Demography & Cover/Summary Data/Site/BenthicREA_sitedata_GENUS_2023.csv",row.names = F)
write.csv(site.data.sp2,file="T:/Benthic/Data/REA Coral Demography & Cover/Summary Data/Site/BenthicREA_sitedata_SPCODE_2023.csv",row.names = F)
write.csv(site.data.tax2,file="T:/Benthic/Data/REA Coral Demography & Cover/Summary Data/Site/BenthicREA_sitedata_TAXONCODE_2023.csv",row.names = F)
