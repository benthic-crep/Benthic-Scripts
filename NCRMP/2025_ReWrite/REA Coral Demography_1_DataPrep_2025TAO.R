...
## LOAD benthic data
loadname=load("T:/Benthic/Data/REA Coral Demography & Cover/Raw from Oracle/ALL_REA_ADULTCORAL_RAW_2013-2023.rdata") #from oracle
x<-df #leave this as df

#Add zeros to beginning of site number so we avoid MAR-22 changing to March 22
x$SITE<- as.factor(x$SITE)
x$SITE<-SiteNumLeadingZeros(x$SITE)  #function in "fish-paste core_functions"
...
#Run two formats
DATE_temp <- mdy(survey_master$DATE_)
DATE_temp[is.na(DATE_temp)]=mdy_hms(survey_master$DATE_[is.na(DATE_temp)])
#check outcome
length(which(is.na(DATE_temp)))
survey_master$DATE_=DATE_temp
class(survey_master$DATE_)
...
#2025 TAO: THERE ARE A WHOLE LOT OF OLD ROLL UP TO GENUS...
...
#test<-new_Aggregate_InputTable(x, SURVEY_INFO) # A very silly function indeed. 
test<-x %>% dplyr::select(all_of(SURVEY_INFO)) %>% distinct()
...
levels(x$GENRD1)<-c(tmp.lev, "NONE") # add to NONE
