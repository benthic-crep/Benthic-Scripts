...
#Calculate survey weights (inverse prorporation weighting)
w.df<-unique(jcdG_stS[,c("ANALYSIS_YEAR","STRATANAME","n","Ntot")])
w.df$sw<-w.df$Ntot/w.df$n
data.gen_temp$ANALYSIS_YEAR<-as.factor(data.gen_temp$ANALYSIS_YEAR)
  
site.sw<-left_join(data.gen_temp,w.df) #merge weights with site-level data
head(site.sw)
site.swS<-subset(site.sw,GENUS_CODE=="SSSS")
write.csv(site.swS,file="T:/Benthic/Projects/Juvenile Project/JuvProject_SITE_weights.csv",row.names = F)
...
#these values are lower than weighted regional means using area estimates. 
