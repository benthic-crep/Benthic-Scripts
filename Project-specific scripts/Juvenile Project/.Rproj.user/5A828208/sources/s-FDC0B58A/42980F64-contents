...
ben<-survey_master%>% dplyr::filter(OBS_YEAR>=2015 & REGION=="SAMOA" & Benthic==1)
#ben<-survey_master%>% dplyr::filter(OBS_YEAR>=2015 & REGION=="PRIAs" & Benthic==1)

ben_days<-ddply(ben,.(OBS_YEAR,ISLAND),summarize,DAYS_N=length(unique(DATE_,na.rm=T)))
ben_days

ben_daysW<-ben_days %>% pivot_wider(names_from=OBS_YEAR, values_from=DAYS_N,values_fill=NA)

#write.csv(ben_daysW,file="T:/Cruise/CruisePreps/2020/RA-20-01 (MARAMP)/Team Plans/2015-2018_PRIAs_Benthicdaysbysector.csv")
