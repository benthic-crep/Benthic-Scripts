...
#LOAD DATA
df<-read.csv("T:/Benthic/Projects/Juvenile Project/JuvDeltaDen_Pred_SITE.csv")#Combined juvenile delta density and all predictors
...
#Rename Predictors
df$Depth<-df$new_MAX_DEPTH_M
df$MaxDHW3<-df$DHW.MaxMax_Degree_Heating_Weeks_YR03
df$MaxDHW10<-df$DHW.MaxMax_Degree_Heating_Weeks_YR10
df$DHW_Freq<-df$DHW.Np10y_Major_Degree_Heating_Weeks_YR10
df$SST_Range<-df$mean_monthly_range_SST_CRW_Daily_YR10

#Extract predictors and merge with new survey weights dataset
pcols<-c("SITE","CORAL", "CCA","TURF","SAND_RUB","Depth","LATITUDE",
         "MaxDHW3","MaxDHW10","DHW_Freq","SST_Range", "WavePower","YearSinceDHW4")

p<-df[,pcols]

#Combine survey weighted juvenile data and predictors
rcols<-c("REGION","SITE","TRANSECTAREA_j","JuvColCount","n","Ntot","sw")
