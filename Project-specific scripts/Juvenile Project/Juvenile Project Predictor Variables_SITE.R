#start by extracting habitat info
...
#LOAD DATA
jwd_site<-read.csv("T:/Benthic/Projects/Juvenile Project/JuvProject_SITE_weights.csv")
...
jwd_site$SEC_NAME<-jwd_site$SEC_NAME
jwd_site$STRATANAME<-paste(jwd_site$SEC_NAME,jwd_site$REEF_ZONE,jwd_site$DEPTH_BIN,sep="_")
...
#Only include sites from most recent surveys
...
#Temporary fix for Palmyra and Kingman- raw DHW data are incorrect for these 2 reefs
ts_sum$YearSinceDHW4<-ifelse(ts_sum$ISLAND=="Kingman",2.1428571,ts_sum$YearSinceDHW4)
ts_sum$YearSinceDHW4<-ifelse(ts_sum$ISLAND=="Palmyra",2.1428571,ts_sum$YearSinceDHW4)
ts_sum$YearSinceDHW4<-ifelse(ts_sum$ISLAND=="Tinian",NA,ts_sum$YearSinceDHW4)

ts_sum$YearSinceDHW8<-ifelse(ts_sum$ISLAND=="Kingman",NA,ts_sum$YearSinceDHW8)
ts_sum$YearSinceDHW8<-ifelse(ts_sum$ISLAND=="Palmyra",2.221429,ts_sum$YearSinceDHW8)
ts_sum$YearSinceDHW4<-ifelse(ts_sum$ISLAND=="Tinian",NA,ts_sum$YearSinceDHW4)
...
#WE ARE MISSING A LOT OF HUMAN DENSITY DATA- LOOK INTO THIS
SM2<-SM%>%dplyr::filter(OBS_YEAR>=2013)

SM2$SEC_NAME<-SM2$SEC_NAME
SM2$STRATANAME<-paste(SM2$SEC_NAME,SM2$REEF_ZONE,SM2$DEPTH_BIN,sep="_")
...
OUTPUT_LEVEL<-c("REGION","ISLAND","SEC_NAME","STRATANAME","DEPTH_BIN") 
...
colnames(dpst)[which(colnames(dpst) == 'SEC_NAME')]<-"SEC_NAME" 
...
jwd_site<-jwd_site %>% mutate(Hab_simple=recode(HABITAT_CODE, 
                                                  `AGR`="Carbonate",
                                                  `APR`="Carbonate",
                                                  `APS`="Carbonate",
                                                  `WAL`="Carbonate",
...
hab_sum<-jwd_site %>%
    group_by(REGION,ISLAND,SEC_NAME,STRATANAME,Hab_simple) %>%
...
jwd_site$MidDepth<-(jwd_site$new_MAX_DEPTH_M+jwd_site$new_MIN_DEPTH_M)/2
depth_sum<-jwd_site %>%
  group_by(REGION,ISLAND,SEC_NAME,STRATANAME) %>%
  summarize(MeanDepth=mean(MidDepth,na.rm=T),MeanMaxDepth=mean(new_MAX_DEPTH_M,na.rm=T))
...
islands = na.omit(unique(jwd_site$ISLAND))#Create vector of island names from your dataframe
islands
df_utm = NULL #create empty dataframe

for (isl in 1:length(islands)) {
  
  df_i = jwd_site %>% subset(ISLAND %in% islands[[isl]])
...
# lat_mean = df %>%
#   group_by(REGION,OBS_YEAR,ISLAND,SEC_NAME,STRATANAME)%>%
