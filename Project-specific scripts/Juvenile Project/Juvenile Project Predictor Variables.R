...
sectors$STRATANAME<-paste(sectors$SEC_NAME,sectors$REEF_ZONE,sectors$DEPTH_BIN,sep="_") #Create stratum
wsd<-left_join(wsd,sectors[,c("STRATANAME","AREA_HA")]);nrow(wsd);head(wsd)
...
#Merge together wsd and sectors
sectors$STRATANAME<-paste(sectors$SEC_NAME,sectors$REEF_ZONE,sectors$DEPTH_BIN,sep="_") #Create stratum
wsd<-left_join(sh.site,sectors[,c("STRATANAME","AREA_HA")]);nrow(wsd);head(wsd)
...
#Clean up- remove SE columns and remove "Mean" from column names
dpst<-dpst %>% dplyr::select(Mean.REGION:Mean.MEAN_SH)

dpst<-dpst %>%
  dplyr::rename_all(funs(stringr::str_replace_all(., "Mean.", "")))
head(dpst)

write.csv(dpst, file="C:/Users/Courtney.S.Couch/Documents/Courtney's Files/R Files/ESD/Juvenile Project/Predictor Variables/JuvProject_SubstrateHeight_STRATA.csv")
