...
df <- read.csv("C:/Users/Jonathan.Charendoff/Documents/GitHub/Benthic-Scripts/SfM/Marianas 2022/Data/V0_CORAL_OBS_E__DATA_VIEW.csv")
...
x<-subset(x,REGION=="MARIAN" & OBS_YEAR =="2022") #subset just MHI 2019 data
stringi::stri_sub(x$SITE[which(nchar(x$SITE) == 7)], 5, 4) <- "00"
stringi::stri_sub(x$SITE[which(nchar(x$SITE) == 8)], 5, 4) <- "0"
...
awd_pre <- x %>% filter(DATE_ < as.Date('2019-07-11'))
awd_post<-x %>% filter(DATE_ >= as.Date('2019-07-11'))
...
awd.[awd.$CONDITION_1 =='BLE' & awd.$SEVERITY_1=='1', cols] <- NA
cols <- c("CONDITION_2", "EXTENT_2", "SEVERITY_2")
awd.[awd.$CONDITION_2 =='BLE' & awd.$SEVERITY_2=='1', cols] <- NA
cols <- c("CONDITION_3", "EXTENT_3", "SEVERITY_3")
awd.[awd.$CONDITION_3 =='BLE' & awd.$SEVERITY_3=='1', cols] <- NA

nrow(x)
nrow(awd.);head(awd.)
x<-awd.; rm("awd.") #remove temporary dataframe if all good. 
...
#Change names of depth columns in SM 
...
x<- inner_join(x, survey_master[survey_master$OBS_YEAR == 2022,c("OBS_YEAR","SITEVISITID","SITE","SEC_NAME","ANALYSIS_YEAR","bANALYSIS_SCHEME","MIN_DEPTH_M","MAX_DEPTH_M")], by=c("OBS_YEAR","SITEVISITID","SITE"))
...
x.na<-x[is.na(x$NO_SURVEY_YN)&as.numeric(x$OBS_YEAR)>2013,]
x.na
...
awd<-droplevels(x)
write.csv(awd,file="T:/Benthic/Data/SfM/Analysis Ready/MARAMP22_DIVERAdult_CLEANED.csv",row.names = F)
...
df <- read.csv("C:/Users/Jonathan.Charendoff/Documents/GitHub/Benthic-Scripts/SfM/Marianas 2022/Data/V0_CORAL_OBS_F_DATA_VIEW.csv")
...
jwd<-droplevels(x)
write.csv(jwd,file="T:/Benthic/Data/SfM/Analysis Ready/MARAMP22_DIVERJuv_CLEANED.csv",row.names = F)
...
df<-read.csv("T:/Benthic/Data/SfM/QC/ASRAMP2023_QCdsfm_ADULT.csv")
...
colnames(x)[colnames(x)=="OLD_DEAD"]<-"OLDDEAD" #Change column name
...
x$OBS_YEAR <- as.vector(rep(2023, times = nrow(x)))
...
stringi::stri_sub(x$SITE[which(nchar(x$SITE) == 7)], 5, 4) <- "00"
stringi::stri_sub(x$SITE[which(nchar(x$SITE) == 8)], 5, 4) <- "0"
survey_master<-read.csv("C:/Users/jonathan.charendoff/Documents/GitHub/Benthic-Scripts/Survey Master Prep/SURVEY_MASTER_w2013benthic.csv")
...
x$SPCODE<-ifelse(x$NO_COLONY==-1,"AAAA",as.character(x$SPCODE)) #currently no sites have -1 for adults (1/22/2020)
...
#Reorder columns
x<-x[,c("METHOD","ANALYST", "REGION","OBS_YEAR","MISSIONID","ISLAND","SEC_NAME","SITEVISITID","SITE","TRANSECT", "REEF_ZONE","DEPTH_BIN",
...
awd<-droplevels(x)
write.csv(awd,file="T:/Benthic/Data/SfM/Analysis Ready/ASRAMP23_SfM_Adult_CLEANED.csv",row.names = F)

#Check number of Site-Segments that contain at least 2 annotators ----------------------------------------------------
analyst.per.ss<-x %>% filter(ANALYST=="JC" | ANALYST=="MSL" | ANALYST=="CA")
...
df<-read.csv("T:/Benthic/Data/SfM/QC/MARAMP2022_QCdsfm_JUV.csv")
...
colnames(x)[colnames(x)=="Shape_Length"]<-"COLONYLENGTH" #Change column name
colnames(x)[colnames(x)=="FID"]<-"COLONYID" #Change column name
colnames(x)[colnames(x)=="MISSION_ID"]<-"MISSIONID" 
...
survey_master<-read.csv("C:/Users/Jonathan.Charendoff/Documents/GitHub/fish-paste/data/SURVEY MASTER_2022only.csv")
survey_master <- read.csv("SURVEY MASTER.csv")
...
x$SPCODE<-ifelse(x$NO_COLONY==-1,"AAAA",as.character(x$SPCODE)) #add S_order column
...
#Reorder columns
x<-x[,c("METHOD","ANALYST", "REGION","OBS_YEAR","ISLAND","SEC_NAME","SITEVISITID","SITE","TRANSECT", "REEF_ZONE","DEPTH_BIN",
        "HABITAT_CODE","LATITUDE","LONGITUDE","MIN_DEPTH_M","MAX_DEPTH_M","SEGMENT","SEGLENGTH","SEGWIDTH",
        "SEGAREA","COLONYID","FRAGMENT","S_ORDER","GENUS_CODE","SPCODE","TAXONCODE","TAXONNAME",
...
jwd<-droplevels(x)
write.csv(jwd,file="T:/Benthic/Data/SfM/Analysis Ready/MARAMP22_SfM_Juv_CLEANED.csv",row.names = F)
