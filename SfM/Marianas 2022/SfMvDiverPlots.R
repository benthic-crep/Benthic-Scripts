...
data.gen_tmp<-read.csv("C:/Users/Jonathan.Charendoff/Documents/GitHub/Benthic-Scripts/SfM/Marianas 2022/Output/MARAMP_repeats_GENUS_Summarized Data.csv")
div.data<-read.csv("C:/Users/Jonathan.Charendoff/Documents/GitHub/Benthic-Scripts/SfM/Marianas 2022/Output/MARAMP_repeats_DIVERSITY_Summarized Data.csv")
jcd.gen<-read.csv("C:/Users/Jonathan.Charendoff/Documents/GitHub/Benthic-Scripts/SfM/Marianas 2022/Output/MARAMP_repeats_GENUS_JUVS_Summarized Data.csv")
...
jcd.gen<-subset(jcd.gen,select = -c(METHOD.1))
nrow(data.gen_tmp)
...
#JUVS
sfm2<-jcd.gen[jcd.gen$MethodRep=="SfM_2",] 
length(unique(sfm2$SS)) #44
sfm1<-jcd.gen[jcd.gen$MethodRep=="SfM_1",]
length(unique(sfm1$SS)) #44
diver1<-jcd.gen[jcd.gen$MethodRep=="Diver_1",]
length(unique(diver1$SS)) #689
diver2<-jcd.gen[jcd.gen$MethodRep=="Diver_2",]
length(unique(diver2$SS)) #43
...
jcd.gen$SST=paste0(jcd.gen$SS,"_",jcd.gen$GENUS_CODE)
seg4list.j=ddply(jcd.gen,.(SST,SS),summarize,NBox=length(unique(MethodRep)))
all4seglist.j=subset(seg4list.j,NBox>=4)
length(unique(all4seglist.j[,"SS"]))
...
juv.sm=subset(jcd.gen,SST%in%all4seglist.j$SST)
dim(juv.sm)
length(unique(juv.sm$SS))

#Check that all Site_Segments being used for analysis have 2 annotators and 2 divers
ddply(juv.sm,.(SITE, SEGMENT), summarize, num.repeats = n_distinct(MethodRep)) #should be 4
ddply(juv.sm,.(SITE, SEGMENT), summarize, num.repeats = n_distinct(METHOD)) #should = 2
...
d1<-data.sm[data.sm$MethodRep=="Diver_1",];colnames(d1)[9:19] <- paste("d1", colnames(d1[9:19]), sep = "");d1<-subset(d1,select=-c(METHOD,SEGAREA_ad,MethodRep,TRANSECT))
d2<-data.sm[data.sm$MethodRep=="Diver_2",];colnames(d2)[9:19] <- paste("d2", colnames(d2[9:19]), sep = "");d2<-subset(d2,select=-c(METHOD,SEGAREA_ad,MethodRep,TRANSECT))
sfm1<-data.sm[data.sm$MethodRep=="SfM_1",];colnames(sfm1)[9:19] <- paste("SfM1", colnames(sfm1[9:19]), sep = "");sfm1<-subset(sfm1,select=-c(METHOD,SEGAREA_ad,MethodRep,TRANSECT))
sfm2<-data.sm[data.sm$MethodRep=="SfM_2",];colnames(sfm2)[9:19] <- paste("SfM2", colnames(sfm2[9:19]), sep = "");sfm2<-subset(sfm2,select=-c(METHOD,SEGAREA_ad,MethodRep,TRANSECT))
...
jd.comp<-ErrorComparision(juv.sm,"GENUS_CODE","JuvColDen")
...
chro.comp<-ErrorComparision(data.sm,"GENUS_CODE","CHRO_prev")
...
all.rmse<-rbind(ad.comp,jd.comp,cl.comp,od.comp,rd.comp, dz.comp, chro.comp, ble.comp,div.comp,rich.comp)
...
ssss.err$Metric=factor(ssss.err$Metric,
                       levels=c("AdColDen","JuvColDen","BLE_prev","Ave.cl","Ave.od","Ave.rd", "DZGN_G_prev", "CHRO_prev", "Shannon","Richness"))
...
metricnames<-data.frame(Metric=c("AdColDen","Ave.cl","Ave.od","BLE_prev","JuvColDen","Richness","Shannon", "Ave.rd", "DZGN_G_prev", "CHRO_prev"),
...
SSSSmet=subset(data.sm,GENUS_CODE=="SSSS")[,c("SS","MethodRep","AdColDen",
                                              "BLE_prev","Ave.cl","Ave.od","Richness","Shannon")]

SSSSmet.j=subset(juv.sm,GENUS_CODE=="SSSS")[,c("SS","MethodRep","JuvColDen")]

Smm.a=melt(SSSSmet,id.vars = c("SS","MethodRep"))
Smm.j=melt(SSSSmet.j,id.vars = c("SS","MethodRep"))
Smm <- rbind(Smm.a, Smm.j)
...
ggsave(plot=Boxes,
       file="C:/Users/Jonathan.Charendoff/Documents/GitHub/Benthic-Scripts/SfM/Marianas 2022/Output/PLOTS/Violin_Comparision.jpeg",
       width=10,height=6)
