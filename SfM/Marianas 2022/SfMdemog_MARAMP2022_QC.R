...
v2 <- read.csv("Data/RA2301_demographics.csv")
sitelist <- read.csv("C:/Users/Jonathan.Charendoff/Documents/GitHub/Benthic-Scripts/SfM/Method Comparision/ASRAMP2023_SfM_Meta.csv")[1:29,]
...
#sfm.raw$SITE[which(sfm.raw$SITE == "A")] <- "GUA-2737"
#sfm.raw$TRANSECT[which(sfm.raw$SITE == "GUA-2737")] <- "A"
#sfm.raw$SITE[which(sfm.raw$SITE == "ROT-869")] <- "ROT-879"
#sfm.raw[which(sfm.raw$SITE == ""),2:10] <- c("JC", 2022, "RA2201", "GUA-2745", "A", 0, 1,1,0)
...
sfm.raw$SITE[sfm.raw$SITE == "TUT-5861"] <- "TUT-5961"
...
#fix errors
sfm.raw$MORPH_CODE[608] <- "MD"
sfm.raw$MORPH_CODE[4860] <- "BR"
#sfm.raw$MORPH_CODE[11649] <- "EM"
#sfm.raw$SPCODE[sfm.raw$ï..OID_ == 8869] <- "ASTS"
#sfm.raw <- sfm.raw[!(sfm.raw$SITE == "MAU-1245" & sfm.raw$ANALYST == "CA" & sfm.raw$Shape_Length >=0.05),]
...
sfm[which(sfm$JUVENILE == -1),"OLDDEAD"] <- 0
sfm[which(sfm$JUVENILE == -1),c(19,20)] <- NA

sfm$JUVENILE[which(sfm$JUVENILE == -1 & sfm$Shape_Length >= 0.05 & sfm$SEGLENGTH > 1)] <- 0
...
sfm$SEGWIDTH[is.na(sfm$SEGAREA)]<-1
...
output[3,]<-c("All transects = A","B sites = repeats") #change depending on output from previous lines of code
...
sfm$SPCODE[sfm$SPCODE != "NA" & sfm$NO_COLONY == -1] <- NA
sfm$MORPH_CODE[sfm$MORPH_CODE != "NA" & sfm$NO_COLONY == -1] <- NA
...
sfm$SPCODE[sfm$SPCODE == "CYSP"] <- "CYPS"
sfm$SPCODE[sfm$SPCODE == "PGRC"] <- "PGWC"
sfm$SPCODE[sfm$SPCODE == "TUSP"] <- "TURS"
sfm$SPCODE[sfm$SPCODE == "STYS"] <- "SPIS"

#suspect codes
suspect <- c(codes$SPCODE[codes$temp <= 5], "CYSP")

Full.suspect <- sfm[sfm$SPCODE %in% suspect,]
Full.suspect <- Full.suspect[!(Full.suspect$SPCODE == "CYSP" & Full.suspect$MORPH_CODE == "FR"),]

good.codes <- read.csv("C:/Users/Jonathan.Charendoff/Documents/Doublecheck_codes - Doublecheck_codes.csv")
good.codes <- arrange(good.codes, good.codes$ï..OID_)
sfm$SPCODE[which(sfm$ï..OID_ %in% good.codes$ï..OID_)] <- good.codes$SPCODE 
...
sm.colonies.eval <- sfm %>% filter(JUVENILE== 0,SEGAREA == 1); View(sm.colonies.eval)
lg.colonies.eval <- sfm %>% filter(JUVENILE==0,SEGAREA==1); View(lg.colonies.eval)
...
#sfm$EXTENT_1[sfm$ï..OID_ == 2447] <- 20
#sfm$SEV_1[sfm$ï..OID_ == 2447] <- 3
...
sfm$SEGLENGTH[which(sfm$REMNANT == -1 & sfm$SEGLENGTH != 2.5)]  #fix seglength error
#sfm$SEGAREA <- sfm$SEGLENGTH*sfm$SEGWIDTH

sfm$REMNANT[which(sfm$REMNANT == -1 & sfm$OLD_DEAD != 0)] <- 0 #fix olddead error
...
juv.sub <- sfm %>% filter(JUVENILE == -1 & is.na(JUV_SUBSTRATE))
...
MSL <- read.csv("C:/Users/Jonathan.Charendoff/Documents/MSL.csv")
sfm$TRANSECT[which(sfm$SITE %in% MSL$SITE & sfm$ANALYST == "MSL")] <- "B" 
sfm$COLONYID<-c(1:length(sfm$ï..OID_))
