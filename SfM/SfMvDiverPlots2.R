...
MyMerge <- function(x, y){
  df <- merge(x, y, by= c("SITE","SITEVISITID","SEGMENT","GENUS_CODE","SS","OBS_YEAR","REGION","ISLAND","SEC_NAME","REEF_ZONE","DEPTH_BIN","HABITAT_CODE",
                          "LATITUDE","LONGITUDE","MIN_DEPTH_M","MAX_DEPTH_M"), all.x=TRUE, all.y= TRUE)
  return(df)
}
df.all<-Reduce(MyMerge, list(d1,d2,sfm1,sfm2));
head(df.all)
nrow(df.all)
...
p1<-PlotAll(df.all,"d1AdColDen","SfM1AdColDen","SfM Adult Density","Count", "Diver1 Adult Density","SfM1 Analyst - Diver1")
p2<-PlotAll(df.all,"d1AdColDen","d2AdColDen","Diver1 Adult Density","Count","Diver2 Adult Density","Diver 2- Diver 1")
p3<-PlotAll(df.all,"SfM1AdColDen","SfM2AdColDen","SfM1 Adult Density","Count","SfM2 Adult Density","SfM2 Analyst - SfM1 Analyst")

outpath<-"T:/Benthic/Data/SfM/ComparisionPlots/Juvenile Density"
p4<-PlotAll(df.all,"d1JuvColDen","SfM1JuvColDen","SfM Juvenile Density","Count", "Diver Juvenile Density","SfM1 Analyst - Diver 1")
p5<-PlotAll(df.all,"d1JuvColDen","d2JuvColDen","Diver1 Juvenile Density","Count","Diver2 Juvenile Density","Diver 2- Diver 1")
p6<-PlotAll(df.all,"SfM1JuvColDen","SfM2JuvColDen","SfM1 Juvenile Density","Count","SfM2 Juvenile Density","SfM2 Analyst - SfM1 Analyst")

outpath<-"T:/Benthic/Data/SfM/ComparisionPlots/Colony Size"
p7<-PlotAll(df.all,"d1Ave.cl","SfM1Ave.cl","SfM Colony Length","Count", "Diver Colony Length","SfM1 Analyst - Diver 1")
p8<-PlotAll(df.all,"d1Ave.cl","d2Ave.cl","Diver1 Colony Length","Count","Diver2 Colony Length","Diver 2- Diver 1")
p9<-PlotAll(df.all,"SfM1Ave.cl","SfM2Ave.cl","SfM1 Colony Length","Count","SfM2 Colony Length","SfM2 Analyst - SfM1 Analyst")

outpath<-"T:/Benthic/Data/SfM/ComparisionPlots/Old Dead"
p10<-PlotAll(df.all,"d1Ave.od","SfM1Ave.od","SfM % Old Dead","Count", "Diver % Old Dead","SfM1 Analyst - Diver 1")
p11<-PlotAll(df.all,"d1Ave.od","d2Ave.od","Diver1 % Old Dead","Count","Diver2 % Old Dead","Diver 2- Diver 1")
p12<-PlotAll(df.all,"SfM1Ave.od","SfM2Ave.od","SfM1 % Old Dead","Count","SfM2 % Old Dead","SfM2 Analyst - SfM1 Analyst")

outpath<-"T:/Benthic/Data/SfM/ComparisionPlots/Recent Dead"
p13<-PlotAll(df.all,"d1Ave.rd","SfM1Ave.rd","SfM % Recent Dead","Count", "Diver % Recent Dead","SfM1 Analyst - Diver 1")
p14<-PlotAll(df.all,"d1Ave.rd","d2Ave.rd","Diver1 % Recent Dead","Count","Diver2 % Recent Dead","Diver 2- Diver 1")
p15<-PlotAll(df.all,"SfM1Ave.rd","SfM2Ave.rd","SfM1 % Recent Dead","Count","SfM2 % Recent Dead","SfM2 Analyst - SfM1 Analyst")

outpath<-"T:/Benthic/Data/SfM/ComparisionPlots/Bleaching"
p16<-PlotAll(df.all,"d1BLE_prev","SfM1BLE_prev","SfM % Bleached","Count", "Diver % Bleached","SfM1 Analyst - Diver 1")
p17<-PlotAll(df.all,"d1BLE_prev","d2BLE_prev","Diver1 % Bleached","Count","Diver2 % Bleached","Diver 2- Diver 1")
p18<-PlotAll(df.all,"SfM1BLE_prev","SfM2BLE_prev","SfM1 % Bleached","Count","SfM2 % Bleached","SfM2 Analyst - SfM1 Analyst")

outpath<-"T:/Benthic/Data/SfM/ComparisionPlots/Chronic DZ"
p19<-PlotAll(df.all,"d1CHRO_prev","SfM1CHRO_prev","SfM Chronic DZ Prev","Count", "Diver Chronic DZ Prev","SfM1 Analyst - Diver 1")
p20<-PlotAll(df.all,"d1CHRO_prev","d2CHRO_prev","Diver1 Chronic DZ Prev","Count","Diver2 Chronic DZ Prev","Diver 2- Diver 1")
p21<-PlotAll(df.all,"SfM1CHRO_prev","SfM2CHRO_prev","SfM1 Chronic DZ Prev","Count","SfM2 Chronic DZ Prev","SfM2 Analyst - SfM1 Analyst")

outpath<-"T:/Benthic/Data/SfM/ComparisionPlots/Acute DZ"
p22<-PlotAll(df.all,"d1DZGN_G_prev","SfM1JuvColDen","SfM Acute DZ Prev","Count", "Diver Acute DZ Prev","SfM1 Analyst - Diver 1")
p23<-PlotAll(df.all,"d1DZGN_G_prev","d2DZGN_G_prev","Diver1 Acute DZ Prev","Count","Diver2 Acute DZ Prev","Diver 2- Diver 1")
p24<-PlotAll(df.all,"SfM1DZGN_G_prev","SfM2DZGN_G_prev","SfM1 Acute DZ Prev","Count","SfM2 Acute DZ Prev","SfM2 Analyst - SfM1 Analyst")



#Plotting using Bland-Altman plot- I don't like these as much for these data but I'm saving the code in case we want to use it. 
#########################################
# outpath<-"T:/Benthic/Data/SfM/ComparisionPlots/Adult Density"
