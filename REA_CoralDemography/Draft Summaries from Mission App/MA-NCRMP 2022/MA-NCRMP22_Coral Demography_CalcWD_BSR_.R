...
library(dplyr)
## LOAD benthic data
ad<-read.csv("C:/Users/Courtney.S.Couch/Documents/GitHub/Benthic-Scripts/REA_CoralDemography/Draft Summaries from Mission App/MA-NCRMP 2022/MA-NCRMP-2022-MissionApp_adult_NOCOLONY.csv")
juv<-read.csv("C:/Users/Courtney.S.Couch/Documents/GitHub/Benthic-Scripts/REA_CoralDemography/Draft Summaries from Mission App/MA-NCRMP 2022/MA-NCRMP-2022-MissionApp_juv_NOCOLONY.csv")

#fill in NA values in no.colony.observed
ad$No.colony.observed<-ifelse(is.na(ad$No.colony.observed),0,ad$No.colony.observed)
View(ad)
juv$No.colony.observed<-ifelse(is.na(juv$No.colony.observed),0,juv$No.colony.observed)
View(juv)
...
x$SPCODE<-ifelse(x$No.colony.observed==-1,"AAAA",x$SPCODE)
...
#Remove transect 2 which is for the repeat segs
x$TRANSECT<-1
...
write.csv(awd,file="C:/Users/Courtney.S.Couch/Documents/GitHub/Benthic-Scripts/REA_CoralDemography/Draft Summaries from Mission App/MA-NCRMP 2022/MA-NCRMP22_cleaned.csv")
...
DATA_COLS<-c("MISSIONID","REGION_NAME","ISLAND","SITE","LATITUDE",	"LONGITUDE","REEF_ZONE","DEPTH_BIN","OBS_YEAR",
             ...
             "SEGLENGTH","COLONYID","Taxoncode","COLONYLENGTH","GENUS","S_ORDER","No.colony.observed")
...
jwd<-subset(jwd,GENUS!="Tubstrea")

jwd_hist<-read.csv("T:/Benthic/Data/REA Coral Demography & Cover/Analysis Ready Raw data/CoralBelt_Juveniles_raw_CLEANED.csv")
jwd_hist$Fragment <- 0

jwd_hist<-subset(jwd_hist,TRANSECT %in% c("1","3"))
...
SURVEY_INFO<-c("SITEVISITID", "OBS_YEAR","REGION_NAME", "ISLAND","SITE", "REEF_ZONE",
               "DEPTH_BIN", "LATITUDE", "LONGITUDE","TRANSECT","COLONYID","GENUS_CODE","SPCODE","MORPH_CODE","COLONYLENGTH")
survey_colony<-Aggregate_InputTable(jwd, SURVEY_INFO)

SURVEY_INFO<-c("SITEVISITID", "OBS_YEAR","REGION_NAME", "ISLAND","SITE", "REEF_ZONE",
               "DEPTH_BIN", "LATITUDE", "LONGITUDE","TRANSECT")
survey_transect<-Aggregate_InputTable(jwd, SURVEY_INFO)
...
SURVEY_INFO<-c("SITEVISITID", "OBS_YEAR","REGION_NAME", "ISLAND","SITE", "REEF_ZONE","DATE_",
               "DEPTH_BIN", "LATITUDE", "LONGITUDE")
survey_site_hist<-Aggregate_InputTable(jwd_hist, SURVEY_INFO)
...
jcd.tax.hist<-Calc_ColDen_Transect(jwd_hist,"SPCODE"); colnames(jcd.tax.hist)[colnames(jcd.tax.hist)=="ColCount"]<-"JuvColCount";colnames(jcd.tax.hist)[colnames(jcd.tax.hist)=="ColDen"]<-"JuvColDen"

jcd.tax<-left_join(jcd.tax,survey_site)
jcd.tax.hist<-left_join(jcd.tax.hist,survey_site_hist)

all.jcd<-rbind(jcd.tax,jcd.tax.hist)

marS<-subset(all.jcd,REGION_NAME=="Mariana Archipelago"&SPCODE=="SSSS")

mar.island<-ddply(marS,.(OBS_YEAR,ISLAND),
                  summarize,
                  meanJuvDen=mean(JuvColDen))
wide<-mar.island %>%
  pivot_wider(names_from = OBS_YEAR,values_from = meanJuvDen)
