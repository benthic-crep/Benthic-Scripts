...
st.demo<-read.csv("T:/Benthic/Data/Data Requests/NCRMPViztool/2022/unformatted/TRENDS/BenthicREA_STRATA_TRENDS_Demo_Viztool_2023.csv")
...
sec.demo<-read.csv("T:/Benthic/Data/Data Requests/NCRMPViztool/2022/unformatted/TRENDS/BenthicREA_SECTOR_TRENDS_Demo_Viztool_2023.csv")
...
#Demo Analysis Year
st.demo$ANALYSIS_YEAR<-ChangeAnalysisYear_SEC(st.demo)
sec.demo$ANALYSIS_YEAR<-ChangeAnalysisYear_SEC(sec.demo)
isl.demo$ANALYSIS_YEAR<-ChangeAnalysisYear_ISL(isl.demo)
r.demo$ANALYSIS_YEAR<-ChangeAnalysisYear_REG(r.demo)

#Cover Analysis Year
st.cover$ANALYSIS_YEAR<-ChangeAnalysisYear_SEC(st.cover)
sec.cover$ANALYSIS_YEAR<-ChangeAnalysisYear_SEC(sec.cover)
isl.cover$ANALYSIS_YEAR<-ChangeAnalysisYear_ISL(isl.cover)
r.cover$ANALYSIS_YEAR<-ChangeAnalysisYear_REG(r.cover)
...
isl.demo <- isl.demo %>% mutate(isl.demo,
                                ISLAND= case_when(
                                  ...
                                  TRUE ~ ISLAND))
st.cover <- st.cover %>% mutate(st.cover,
                              ISLAND= case_when(
                                SECTOR %in% c("Guguan", "Alamagan", "Sarigan") ~ "Sarigan, Alamagan, Guguan",
                                TRUE ~ ISLAND))
isl.cover <- isl.cover %>% mutate(isl.cover,
                                ISLAND= case_when(
                                  ISLAND == "SGA" ~ "Sarigan, Alamagan, Guguan",
                                  TRUE ~ ISLAND))
...
write.csv(x = templ_str,file = "C:/Users/Thomas.Oliver/WORK/Projects/FY23/Viztool/PacBenthicStrat_Trend_NJK.csv",)
write.csv(x = templ_sec,file = "C:/Users/Thomas.Oliver/WORK/Projects/FY23/Viztool/PacBenthicSec_Trend_NJK.csv")
write.csv(x = templ_isl,file = "C:/Users/Thomas.Oliver/WORK/Projects/FY23/Viztool/PacBenthicSubjur_Trend_NJK.csv")
write.csv(x = templ_reg,file = "C:/Users/Thomas.Oliver/WORK/Projects/FY23/Viztool/PacBenthicJur_Trend_NJK.csv")
...
REGION_JKlu=sort(c(unique(st.all$JURISDICTIONname)))
...
st.all$JoinKey=paste0("Strata","_",
                      ...
                      st.all$SurveyYearStart,"_",
                      st.all$SurveyYearEnd)
...
st.gen$JoinKey=paste0("Strata","_",
                      ...
                      st.gen$SurveyYearStart,"_",
                      st.gen$SurveyYearEnd)
...
st.both$MonitoringCycle=as.integer(st.both$AnalysisYear)
...
st.both.rn=st.both.rn %>% filter(!(N_demoTREND==0&N_coverTREND==0))
...
templ_str_Ljoin= left_join(x=templ_str[,-which(names(templ_str)%in%TREND_cols)],
                           ...
                           by=join_cols);dim(templ_str_Ljoin)
...
sec.all$JoinKey=paste0("Sector","_",
                       ...
                       sec.all$SurveyYearStart,"_",
                       sec.all$SurveyYearEnd)
...
sec.gen$JoinKey=paste0("Strata","_",
                       ...
                       sec.gen$SurveyYearStart,"_",
                       sec.gen$SurveyYearEnd)
...
sec.both$MonitoringCycle=as.integer(sec.both$AnalysisYear)
...
# Drop any rows with NA/O in all N columns
sec.both.rn=sec.both.rn %>% filter(!(N_demoTREND==0&N_coverTREND==0))
...
isl.all$JoinKey=paste0("Subjurisdiction","_",
                       ...
                       isl.all$SurveyYearStart,"_",
                       isl.all$SurveyYearEnd)
...
isl.gen$JoinKey=paste0("Subjurisdiction","_",
                       ...
                       isl.gen$SurveyYearStart,"_",
                       isl.gen$SurveyYearEnd)
...
isl.both$MonitoringCycle=as.integer(isl.both$AnalysisYear)
...
# Drop any rows with NA/O in all N columns
isl.both.rn=isl.both.rn %>% filter(!(N_demoTREND==0&N_coverTREND==0))
...
r.both$MonitoringCycle=as.integer(r.both$AnalysisYear)
...
# Drop any rows with NA/O in all N columns
r.both.rn=r.both.rn %>% filter(!(N_demoTREND==0&N_coverTREND==0))
